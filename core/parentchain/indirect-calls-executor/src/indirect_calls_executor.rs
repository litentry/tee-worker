		...
		ParentchainBlock: ParentchainBlockTrait<Hash = H256>,
	{
		...
		for xt_opaque in block.extrinsics().iter() {
			...
			if let Ok(xt) = ParentchainUncheckedExtrinsic::<VCSchemaIssuedFn>::decode(
				...
				info!("start check VCSchemaIssuedFn");
				if self.is_vc_schema_issued_function(&xt.function.0) {
					let (_, shard, schema_id, schema_content) = xt.function;
					info!("Found Schema Issued extrinsic in block: \nSchema Id {:?} \nSchema Content: {} \nShard: {}",
					bs58::encode(shard.encode()).into_string(), bs58::encode(shard.encode()).into_string(), bs58::encode(shard.encode()).into_string());
				}
			}
		}
