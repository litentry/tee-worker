...
// litentry query
pub type LinkedEthereumAddress = std::vec::Vec<EthAddress>;
pub type LinkedSubstrateAddress = std::vec::Vec<LinkedSubAccount<AccountId>>;
...
pub fn match_trusted_commands(cli: &Cli, trusted_args: &TrustedArgs) {
	match &trusted_args.command {
		...
		// Litentry
		TrustedCommands::SetUserShieldingKey { account, key } =>
			set_user_shielding_key(cli, trusted_args, account, key),
		...
		TrustedCommands::LinkedSubAddresses { account } =>
			linked_sub_addresses(cli, trusted_args, account),
		TrustedCommands::LinkEth {
			account,
			index,
			eth_address,
